...
export async function traceTask(
  ...
  if (!(await pathExists(graphPath))) {
    throw new Error(
      "architecture/metadata/traceability/graph.json not found. Run 'pa init' or a mutating command first.",
    );
  }

  const tasksPath = path.join(traceabilityRoot, "nodes", "tasks.json");
  const edgesDecisionPath = path.join(traceabilityRoot, "edges", "task_to_decision.json");
  const edgesModulePath = path.join(traceabilityRoot, "edges", "task_to_module.json");
  ...
  if (!task) {
    throw new Error(`Task '${taskRef}' not found in architecture metadata traceability graph`);
  }
