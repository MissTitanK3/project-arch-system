...
import { defaultPhaseManifest } from "../manifests";
...
async function collectPhaseAndMilestoneIssues(
  ...
  const phasesRoot = path.join(roadmapDir, "phases");
  ...
  if (!(await fs.pathExists(phasesRoot))) {
    issues.push(
      buildIssue({
        code: DOCTOR_HEALTH_CODES.MISSING_REQUIRED_ROOT,
        severity: "error",
        message: "Missing required directory 'roadmap/phases'.",
        fix: "Create roadmap/phases directory.",
        repairable: true,
        path: "roadmap/phases",
        repairAction: async () => {
  ...
  const phaseDirectories = await fg("roadmap/phases/*", {
    cwd,
    ...
    if (!(await fs.pathExists(milestonesDir))) {
      issues.push(
        buildIssue({
          ...
          severity: "error",
          message: `Phase '${phaseName}' is missing milestones directory.`,
      ...
      if (!(await fs.pathExists(milestoneManifestPath))) {
        issues.push(
          buildIssue({
            ...
            severity: "error",
            message: `Milestone '${phaseName}/${milestoneName}' is missing manifest.json.`,
      ...
      for (const lane of TASK_LANES) {
        ...
        issues.push(
          buildIssue({
            ...
            severity: "error",
            message: `Milestone '${phaseName}/${milestoneName}' is missing '${lane}' task lane directory.`,
