...
export async function scaffoldGovernanceGuidanceDocs(
  ...
  if (!(await pathExists(repoModelPath))) {
    const repoModel = [
      ...
      "Canonical local init entrypoint is `pa init --mono`.",
  ...
  if (!(await pathExists(initDefaultBehaviorPath))) {
    const initDefaultBehaviorGuide = [
      ...
      "- Tier D optional add-ons",
      "",
      "## First-Run No-Test Baseline",
      "",
      "The default scaffold intentionally supports a fresh-scaffold no-test baseline before workspace packages exist.",
      "",
      "Expected fresh-repo behavior:",
      "",
      "- `task typecheck`, `task lint`, and `task test` may report `No projects matched the filters`",
      "- `task check` emits one intentional baseline message when no workspace packages are present, then executes architecture validation via `pa check` when available",
      "- if `pa` is unavailable on PATH in the current shell, `task check` should emit explicit guidance rather than failing with an opaque executable-not-found error",
      "- `pnpm typecheck`, `pnpm lint`, `pnpm test`, and `pnpm check` should mirror the same behavior because they are Taskfile wrappers",
      "- use `task arch:check:json` (or `pnpm check:json`) when machine-readable architecture diagnostics are required",
      "- all of these commands should still exit successfully in the fresh no-test baseline state",
      "",
      "This baseline is intentional and should not be treated as scaffold breakage.",
      ...
      "",
      "Bootstrap-specific `pa init` should create:",
      "",
  ...
  if (!(await pathExists(cliContextPayloadPath))) {
    const cliContextPayloadGuide = [
      ...
      '    "project": {',
      '      "id": "<project>",',
      '      "path": "roadmap/projects/<project>",',
      '      "title": "Project"',
      "    },",
      '    "phase": {',
      '      "id": "phase-1",',
      '      "path": "roadmap/projects/<project>/phases/phase-1",',
      '      "title": "Phase 1"',
      "    },",
      '    "milestone": {',
      '      "id": "milestone-2",',
      '      "path": "roadmap/projects/<project>/phases/phase-1/milestones/milestone-2",',
      '      "title": "Milestone 2"',
      "    },",
      '    "task": {',
      '      "id": "003-implement-context",',
      '      "path": "roadmap/projects/<project>/phases/phase-1/milestones/milestone-2/tasks/planned/003-implement-context.md",',
      ...
      '    "task": {',
      '      "id": "004-next-task",',
      '      "path": "roadmap/projects/<project>/phases/phase-1/milestones/milestone-2/tasks/planned/004-next-task.md",',
      '      "title": "Next Task",',
      '      "status": "todo"',
      "    }",
      "  }",
...
export async function scaffoldAgentsGuide(
  ...
  if (!(await pathExists(agentsDocPath))) {
    const agentsDoc = [
      ...
      "",
      "1. `architecture/metadata/codebase-map/README.md`",
      "",
      "   - Machine-readable codebase topology and module navigation.",
      "",
      "1. `architecture/metadata/traceability/graph.json`",
      "",
      ...
      "",
      "1. `architecture/metadata/domains/README.md`",
      "",
      ...
      "1. `architecture/standards`",
      "1. `architecture/metadata/domains`",
      "1. `roadmap/decisions`",
      "1. `roadmap/projects`",
      "1. `architecture/templates`",
